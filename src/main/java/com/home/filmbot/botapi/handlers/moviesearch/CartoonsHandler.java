...
@Slf4j
@Component
public class CartoonsHandler implements InputMessageHandler {
    private final ReplyMessagesService messagesService;
    private final MenuService menuService;

    public CartoonsHandler(ReplyMessagesService messagesService, MenuService menuService) {
    ...
    public SendMessage handle(Message message) {
        if (message.getText().equals("Мультфильмы")) {
    ...
    public BotState getHandlerName() {
        return BotState.SEARCH_CARTOONS;
    }
